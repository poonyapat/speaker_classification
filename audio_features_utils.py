...
### Parameters ###
# fft_size = 1600  # window size for the FFT
# step_size = fft_size // 16  # distance to slide along the window (in time)
# spec_thresh = 8  # threshold for spectrograms (lower filters out more noise)
# lowcut = 500  # Hz # Low cut for our butter bandpass filter
# highcut = 7000  # Hz # High cut for our butter bandpass filter
# # For mels
# n_mel_freq_components = 196  # number of mel frequency channels
...
fft_size = 800  # window size for the FFT
...
def recover_spectrogram(spectrogram):
    return invert_pretty_spectrogram(
        wav_spectrogram, fft_size=fft_size, step_size=step_size, log=True, n_iter=10
    )

def to_mel_spectrogram(data, rate):
    wav_spectrogram = to_spectrogram(data, rate)
    return make_mel(wav_spectrogram, mel_filter, shorten_factor=shorten_factor)
    
def recover_mel_spectrogram(mel_spec):
